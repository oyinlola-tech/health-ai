...
describe("aiGateway cost governance", () => {
  it("routes medical reports to Pro, calls Gemini, caches, and logs costed usage", async () => {
    ...
    expect(result.model).toBe("gemini-test-pro");
    expect(mocks.generateContent).toHaveBeenCalledOnce();
    expect(mocks.storeCache).toHaveBeenCalledWith(expect.objectContaining({ featureType: "report_analysis", modelUsed: "gemini-test-pro" }));
