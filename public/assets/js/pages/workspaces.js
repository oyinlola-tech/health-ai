...
const adminSections = [
  ...
  { label: "Revenue Analytics", href: "/admin/revenue", icon: "monitoring", group: "Analytics" },
...
async function renderAdminDashboard() {
  ...
  const meta = { title: section.label, description: adminDescription(section.href) };
...
function OpsHeader(meta) {
  return `
    <section class="ops-header">
      <div>
        <p class="eyebrow">MedExplain AI Operations</p>
        <h1>${escapeHtml(meta.title)}</h1>
        <p class="lead">${escapeHtml(meta.description)}</p>
      </div>
      <div class="ops-header-actions">
        <a class="btn btn-secondary" href="/admin/audit-logs">${icon("fact_check")}Audit trail</a>
        <button class="btn btn-primary" type="button" data-action="retry">${icon("refresh")}Refresh</button>
      </div>
    </section>
  `;
}
...
function adminDescription(route) {
  return {
    ...
    "/admin/system-health": "Report processing, infrastructure posture, API health, and operational readiness."
  }[route];
}
...
function renderAiOperations(context) {
  ...
  return SectionScaffold({
    ...
    table: DataTable({ title: "Feature cost breakdown", description: "AI usage grouped by backend feature.", rows, columns: [["feature", "Feature"], ["requests", "Requests"], ["cost", "Cost"], ["status", "Status"]], searchKey: "feature", emptyKey: "ai", actions: [{ label: "Refresh AI costs", href: "/admin/ai-operations" }] })
  });
}
...
function renderSecurityCenter(context) {
  ...
  return SectionScaffold({
    stats: [
      ...
      StatCard("Audit Coverage", context.logs.length, "Recorded events", "fact_check", "Evidence")
    ],
...
function renderComplianceCenter(context) {
  ...
  return SectionScaffold({
    stats: [
      ...
      StatCard("Audit Evidence", context.logs.length, "Operational records", "fact_check", "Traceability"),
...
function renderSupportCenter(context) {
  ...
  return SectionScaffold({
    stats: [
      ...
      StatCard("Recent Events", context.logs.length, "Audit activity", "fact_check", "Context"),
...
function renderAuditLogs(context) {
  return SectionScaffold({
    stats: [
      StatCard("Audit Logs", context.logs.length, "Recorded events", "fact_check", "Evidence"),
    ...
    table: DataTable({ title: "Audit trail", description: "Search and filter persisted audit logs.", rows: logRows(context.logs), columns: [["action", "Action"], ["actor", "Actor"], ["entity", "Entity"], ["time", "Time"]], searchKey: "action", emptyKey: "logs", actions: [{ label: "Compliance center", href: "/admin/compliance" }] })
  });
}
...
function renderSystemHealth(context) {
  ...
  return SectionScaffold({
    ...
    table: DataTable({ title: "System checks", description: "Operational readiness from backend health metrics.", rows, columns: [["system", "System"], ["value", "Value"], ["status", "Status"]], searchKey: "system", emptyKey: "reports", actions: [{ label: "Refresh health", href: "/admin/system-health" }] })
  });
}
...
function renderDoctorSection(section, context) {
  ...
  return SectionScaffold({
    ...
    table: DataTable({ title, description: "Doctor workspace records filtered for this clinical section.", rows, columns: [["name", "Record"], ["detail", "Detail"], ["status", "Status"], ["time", "Time"]], searchKey: "name", emptyKey: "doctors", actions: [{ label: "Refresh workspace", href: section.href }] }),
